...
function toNumber(value: number, fallback: number, min: number) {
  if (!Number.isFinite(value)) return fallback;
...
function normalizeEnduranceSettings(settings: EnduranceSettings): EnduranceSettings {
  return {
    ...
    minTimeBonus: toInteger(settings.minTimeBonus, DEFAULT_ENDURANCE_SETTINGS.minTimeBonus, 0),
    timeBonusStep: toInteger(settings.timeBonusStep, DEFAULT_ENDURANCE_SETTINGS.timeBonusStep, 0),
    maxTimeMultiplier: toNumber(settings.maxTimeMultiplier, DEFAULT_ENDURANCE_SETTINGS.maxTimeMultiplier, 1),
